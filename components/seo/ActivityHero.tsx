...
export function ActivityHero({ activity, guideCount = 0, cityCount = 0 }: ActivityHeroProps) {
      ...
      <div className="relative container mx-auto px-5 py-20 md:py-32">
        <div className="max-w-4xl">
          ...
          <div className="inline-flex items-center gap-2 bg-green-500/20 border border-green-500/30 rounded-full px-4 py-1.5 mb-6 backdrop-blur-sm">
            ...
            <span className="text-green-300 text-sm font-medium">Experiencias Auténticas</span>
          </div>
          
          {/* Title */}
          <h1 className="text-4xl md:text-6xl font-bold mb-6 tracking-tight leading-tight">
            Tours de <span className="text-green-400">{activity.name}</span>
          </h1>
            ...
            <div className="flex flex-wrap gap-8 text-sm">
              {guideCount > 0 && (
                <div>
                  ...
                  <div className="text-gray-400">Guías Especializados</div>
                </div>
              )}
              {cityCount > 0 && (
                <div>
                  <div className="text-3xl font-bold text-green-400">{cityCount}</div>
                  <div className="text-gray-400">Destinos Disponibles</div>
                </div>
              )}
            </div>
          )}
